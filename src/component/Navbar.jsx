...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#F7FCFC] shadow-[0_0.25rem_2.5rem_rgba(0,0,0,0.08)] px-[1.25rem] sm:px-[1.5rem] md:px-[2rem] lg:mx-[5rem] xl:mx-[6rem] 2xl:mx-[30rem] lg:px-[2.5rem] py-[1rem] sm:py-[1.25rem] rounded-none lg:mt-[2.5rem] lg:rounded-[1.5rem]">
        ...
        <div className="hidden lg:flex items-center gap-[0.5rem] xl:gap-[2rem] 2xl:gap-[3rem] text-[0.875rem] xl:text-[1rem]">
          {navLinks.map((link) => (
            <NavLink
              ...
              to={link.to}
              className={({ isActive }) =>
                `transition-colors relative pb-[0.25rem] ${
                  isActive 
            ...
            >
              {({ isActive }) => (
                <>
                  {link.label}
                  {isActive && (
                    <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[3.4375rem] h-[0.125rem] bg-[#0096FF] rounded-[1.6875rem]"></span>
                  )}
                </>
              )}
            </NavLink>
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden mt-[1rem] pt-[1rem] border-t border-gray-200 flex flex-col gap-[1rem] transition-all duration-300 ease-in-out">
          {navLinks.map((link) => (
            <NavLink
              key={link.to}
              to={link.to}
