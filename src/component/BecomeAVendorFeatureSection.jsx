...
export default function BecomeAVendorFeatureSection({
  heading,
  headingHighlight,
  description,
    ...
    <section className={`relative bg-white ${sectionPadding} overflow-hidden font-[Helvetica,Arial,sans-serif]`}>
      ...
      <div className="max-w-[90rem] mx-auto px-[1rem] sm:px-[1.5rem] md:px-[3rem] lg:px-[5rem] xl:px-[6rem] relative z-10">
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-2 gap-[2rem] lg:gap-[3rem] xl:gap-[4rem] items-center">
          ...
          <div className={`relative w-full ${imagesOrderClasses}`}>
            {/* Phone Mockups */}
            <div className={`relative w-full flex ${imagesJustifyClasses}`}>
            ...
            {/* Secondary Paragraph - Mobile Only (below images) */}
            {descriptionSecondary && (
              <p className="block lg:hidden text-[0.875rem] sm:text-[1rem] md:text-[1.25rem] font-normal leading-[1.5rem] sm:leading-[1.75rem] md:leading-[2.25rem] text-center text-[#00212A] mt-[6.5rem] sm:mt-[6.5rem] md:mt-[15rem] px-[1rem]">
                {descriptionSecondary}
              </p>
            )}
          </div>
          ...
          <div className={`relative space-y-[1.5rem] sm:space-y-[2rem] lg:space-y-[2rem] ${contentOrderClasses} w-full`}>
            ...
            {/* Secondary Paragraph - Desktop Only */}
            {descriptionSecondary && (
              <p className="hidden lg:block text-[1rem] sm:text-[1.125rem] md:text-[1.25rem] lg:text-[1.375rem] xl:text-[1.5rem] font-normal leading-[1.5rem] sm:leading-[1.75rem] md:leading-[2rem] lg:leading-[2rem] xl:leading-[2.25rem] text-left text-[#00212A] max-w-[35rem] lg:max-w-[40rem]">
                {descriptionSecondary}
              </p>
            )}
          </div>
